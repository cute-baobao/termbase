...
const alreadyExists = async (c: Context, next: Next) => {
  ...
  if (result) return c.json({ success: false, message: t('email-already-exists') });
  return next();
};
...
const app = new Hono<AdditionalContext>()
  .post('/signIn', zValidator('json', signInSchema), userExists, async (c) => {
    ...
    const base64Payload = Buffer.from(JSON.stringify(res)).toString('base64');
    // 设置cookie
    setCookie(c, AUTH_COOKIE_NAME, base64Payload, {
      httpOnly: true,
